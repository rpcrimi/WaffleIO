# Waffle.io

## Introduction

Waffle.io is an online Kanban board integrated with GitHub. It was created in 2003 by a group of interns at Rally Software, right here in Boulder. Waffle helps development teams track GitHub tasks, issues, and user stories by using an easy to use GUI and command line interface. Throughout this presentation, we will observe Waffle's abundant features by working through an example project.

## Table of Contents
1. [Kanban](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/kanban.md)
2. [Set-Up](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/set_up.md)
3. [Learning Waffle](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/learn_waffle.md)
4. [Adding Stores](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/adding_stories.md)
5. [Git Integration](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/git_integration.md)
6. [Git Integration: More!](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/git_integration_more.md)
7. [Waffle Advantages](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/waffle_advantages.md)
8. [Conclusion](https://github.com/rpcrimi/WaffleIO/blob/master/markdown/conclusion.md)